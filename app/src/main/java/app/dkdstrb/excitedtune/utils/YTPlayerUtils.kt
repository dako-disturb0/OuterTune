 ...
 *
 * Playback resolution ported from Metrolist:
 * WEB_REMIX main client + poToken/signatureTimestamp + ContentAwareFallbackStrategy,
 * custom cipher deobfuscation with n-transform, and NewPipe fallbacks.
 */
...
import app.dkdstrb.excitedtune.utils.potoken.PoTokenGenerator
import app.dkdstrb.excitedtune.utils.potoken.PoTokenResult
...
object YTPlayerUtils {
    ...
    // stream validation during resolution.
     ...
     * visitorData being ready. Failure is swallowed; playback falls back to lazy init unchanged.
     */
    ...
    suspend fun playerResponseForPlayback(
        ...
        Timber.tag(logTag).d("Authentication status: ${if (isLoggedIn) "LOGGED_IN" else "ANONYMOUS"}")

        // Get signature timestamp (cipher player preferred; NewPipe as detection/fallback)
        ...
        val mainPlayerResponse = YouTube
            .player(
                ...
                playlistId,
                MAIN_CLIENT,
                if (MAIN_CLIENT.useSignatureTimestamp) signatureTimestamp.timestamp else null,
        ...
        val hasHighQuality = mainPlayerResponse?.streamingData?.adaptiveFormats
            ?.any { it.audioQuality == "AUDIO_QUALITY_HIGH" } == true
        ...
        for ((clientIndex, client) in streamClients.withIndex()) {
            ...
            if (streamPlayerResponse?.playabilityStatus?.status == "OK") {
                ...
                val currentMusicVideoType = streamPlayerResponse.videoDetails?.musicVideoType
                ...
                if (needsNTransform) {
                    try {
                        ...
                        throw e // request superseded/cancelled — abort cleanly
                    } catch (e: Exception) {
                ...
                val isUgcOrPodcast = currentMusicVideoType == "MUSIC_VIDEO_TYPE_UGC" ||
                    currentMusicVideoType?.contains("PODCAST") == true ||
                    currentMusicVideoType == null
                ...
                if (validateStatus(streamUrl!!, currentClient.streamHeaders())) {
                    ...
                    // cipher rebuilds its WebView and the next resolution returns to this client.
                    if (needsNTransform) {
    ...
    suspend fun playerResponseForMetadata(
        ...
        val sessionId = YouTube.visitorData
        var poToken: PoTokenResult? = null
        if (YouTubeClient.WEB_REMIX.useWebPoTokens && sessionId != null) {
            try {
                poToken = poTokenGenerator.getWebClientPoToken(videoId, sessionId)
            } catch (_: Exception) {
            }
    ...
    private fun findFormat(
        ...
        if (format != null) {
            Timber.tag(logTag)
                .d("Selected format: itag=${format.itag}, mimeType=${format.mimeType}, bitrate=${format.bitrate}")
        } else {
    ...
    private suspend fun getSignatureTimestampOrNull(videoId: String): SignatureTimestampResult {
        ...
        val cipherSts = try {
            ...
            throw e // cooperative cancellation: don't swallow
        } catch (e: Exception) {
        ...
        return result.fold(
            ...
            onFailure = { error ->
                ...
                when {
                    ...
                    cipherSts != null -> {
                        // Non-fatal: the cipher player's STS already covers us.
